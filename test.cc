...
auto main() -> int {
  ...
  int a[2]{};
  for (int i = 0; i < 2; ++i) std::print("{}\t", a[i]);
  std::print("{}\t\n", __cplusplus);
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int i = 0; i < n; i++) assert(compar[i] == vec[i]);
  ...
  for (int i = 0; i < num; ++i) {
    ...
    int after = vec.capacity();
    if (before != after) {
      for (int i = 0; i < n - 1; i++) assert(compar[i] == vec[i]);
    }
  }
  std::print("\n");
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int i = 0; i < n; i++) assert(compar[i] == deck[i]);
  ...
  for (int i = 0; i < num; ++i) {
    ...
    int after = deck.capacity();
    if (before != after) {
      for (int i = 1; i < n; i++) assert(compar[i] == deck[i - 1]);
    }
  }
  std::print("\n\n");
