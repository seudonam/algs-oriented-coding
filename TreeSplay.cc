...
struct SplayTree {
    ...
    while (!s.empty()) {
      if (s.back() != x->p) {
        while (s.back()) {
          Node *y{s.back()};
          if (y->left) {
            if (y->right) s.push_back(y->right);
            s.push_back(y->left);
          } else
            s.push_back(y->right);
        }
  ...
  bool isBST(Node *x, K &max) const {
    ...
    if (!isBST(x->left, max)) return false;
    if (max > x->key) return false;
    std::print("{} | ", x->key);
    max = x->key;
    if (!isBST(x->right, max)) return false;
    return true;
  }
};
...
int main() {
  ...
  for (const auto &e : a) {
    ...
    if (SplayTree.search(e)) SplayTree.remove(e);
    std::print("\npreWalk\t");
    SplayTree.preWalk();
    std::print("\nposWalk\t");
    SplayTree.postWalk();
