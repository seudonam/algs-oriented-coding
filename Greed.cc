...
template <class T>
using adj_list = std::deque<std::deque<T>>;
...
struct dikstra {
  ...
  dikstra(const adj_list<directed_edge> &graph, int src)
      : marked(graph.size(), false),
        dist_to(graph.size(), 0xffff),
  ...
  auto path_to(int v) const {
    ...
    for (auto e{edge_to[v]}; e; e = edge_to[e->from]) path.push_front(*e);
    return path;
  }
};
...
struct kruskal {
  ...
  kruskal(const std::deque<edge> &graph, size_t num) : weight{0}, parent(num) {
    for (size_t v = 0; v < num; ++v) parent[v] = v;
    std::priority_queue<edge, std::deque<edge>, std::greater<edge>> pq(
...
int main() {
  ...
  for (int k = 0; k < 16; k++) {
    ...
    auto klist{kmst.mst};
    for (const auto &e : klist) std::print("[{}-{}|{}]\t", e.v, e.w, e.weight);
    std::print("\nweight\t{}\n", kmst.weight);
    std::print("\n");

    prim pmst(ewg);
    auto plist{pmst.mst};
    for (const auto &e : plist) std::print("[{}-{}|{}]\t", e.v, e.w, e.weight);
    std::print("\nweight\t{}\n", pmst.weight);
