...
struct TreeMap {
  ...
  void remove(Node *z) {
    ...
    if (y) {
      ...
      if (z->color == black) removeFixup(y);
    } else if (z->p == nullptr)
      root = nullptr;
    else {
      if (z->color == black) removeFixup(z);
      if (z->p) {
        if (z == z->p->left)
          z->p->left = nullptr;
        else if (z == z->p->right)
  ...
  bool isRedBlack(K min, K max) const {
    ...
    if (!isRedBlack(root, min, max, p_of_null)) return false;
  ...
  bool isRedBlack(Node *x, K min, K max, std::vector<Node *> &p_of_null) const {
    ...
    if (color(x) == red) {
      if (color(x->p) != black) return false;
      if (color(x->left) != black || color(x->right) != black) return false;
    }
    if (x->left == nullptr || x->right == nullptr) p_of_null.push_back(x);
...
int main() {
  ...
  for (const auto &e : a) {
    std::print("\nremove\t{}", e);
    if (TreeMap.search(e)) TreeMap.remove(e);
    std::print("\npreWalk\t");
    TreeMap.preWalk();
    std::print("\ninWalk\t");
    TreeMap.inWalk();
