...
struct LeftistHeap {
  ...
  Node *merge(Node *a, Node *b) {
    ...
    if (comparator(a->e, b->e)) std::swap(a, b);
  ...
  bool isLeftistHeap(Node *x) {
    ...
    if (x->left && comparator(x->e, x->left->e)) return false;
    if (x->right && comparator(x->e, x->right->e)) return false;
    if (nullPathLength(x->left) < nullPathLength(x->right)) return false;
